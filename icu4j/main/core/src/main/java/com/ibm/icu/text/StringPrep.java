 ...
 * href="http://www.ietf.org/rfc/rfc3454.txt">RFC 3454</a>. StringPrep prepares Unicode strings for
 * use in network protocols. Profiles of StingPrep are set of rules and data according to which the
 * Unicode Strings are prepared. Each profiles contains tables which describe how a code point
 * should be treated. The tables are broadly classied into
 *
 * <ul>
 *   <li>Unassigned Table: Contains code points that are unassigned in the Unicode Version supported
 *       by StringPrep. Currently RFC 3454 supports Unicode 3.2.
 *   <li>Prohibited Table: Contains code points that are prohibited from the output of the
 *       StringPrep processing function.
 *   <li>Mapping Table: Contains code points that are deleted from the output or case mapped.
 * </ul>
 *
 * The procedure for preparing Unicode strings:
 *
 * <ol>
 *   <li>Map: For each character in the input, check if it has a mapping and, if so, replace it with
 *       its mapping.
 *   <li>Normalize: Possibly normalize the result of step 1 using Unicode normalization.
 ...
 *       that the whole string satisfies the requirements for bidirectional strings. If the string
 *       does not satisfy the requirements for bidirectional strings, return an error.
 * </ol>
 *
...
public final class StringPrep {
    ...
    /* indexes[] value names */
    //  private static final int INDEX_TRIE_SIZE                  =  0; /* number of bytes in
    // normalization trie */
    private static final int INDEX_MAPPING_DATA_SIZE =
            1; /* The array that contains the mapping   */
    private static final int NORM_CORRECTNS_LAST_UNI_VERSION =
            2; /* The index of Unicode version of last entry in NormalizationCorrections.txt */
    private static final int ONE_UCHAR_MAPPING_INDEX_START =
            3; /* The starting index of 1 UChar mapping index in the mapping data array */
    private static final int TWO_UCHARS_MAPPING_INDEX_START =
            4; /* The starting index of 2 UChars mapping index in the mapping data array */
    ...
    /**
     * Creates an StringPrep object after reading the input stream. The object does not hold a
     * reference to the input steam, so the stream can be closed after the method returns.
     *
    ...
    private StringPrep(ByteBuffer bytes) throws IOException {
        ...
        if (normUniVer.compareTo(sprepUniVer) < 0
                && /* the Unicode version of SPREP file must be less than the Unicode Version of the normalization data */ normUniVer
                                .compareTo(normCorrVer)
                        < 0
                && /* the Unicode version of the NormalizationCorrections.txt file should be less than the Unicode Version of the normalization data */ ((indexes[
                                        OPTIONS]
                                & NORMALIZATION_ON)
    ...
    private StringBuffer normalize(StringBuffer src) {
        return new StringBuffer(
                Normalizer.normalize(src.toString(), Normalizer.NFKC, Normalizer.UNICODE_3_2));
    }
    ...
    */
    /**
     * Prepare the input buffer for use in applications with the given profile. This operation maps,
     * normalizes(NFKC), checks for prohibited and BiDi characters in the order defined by RFC 3454
     * depending on the options specified in the profile.
     *
     * @param src A UCharacterIterator object containing the source string
     * @param options A bit set of options:
     *     <ul>
     *       <li>{@link #DEFAULT} Prohibit processing of unassigned code points in the input
     *       <li>{@link #ALLOW_UNASSIGNED} Treat the unassigned code points are in the input as
     *           normal Unicode code points.
     *     </ul>
     *
        ...
        if (checkBiDi == true) {
            ...
            if (leftToRight == true && rightToLeft == true) {
                throw new StringPrepParseException(
                        ...
                        StringPrepParseException.CHECK_BIDI_ERROR,
                        iter.getText(),
            ...
            // satisfy 3
            if (rightToLeft == true
                    && !((firstCharDir == UCharacterDirection.RIGHT_TO_LEFT
    ...
    /**
     * Prepare the input String for use in applications with the given profile. This operation maps,
     * normalizes(NFKC), checks for prohibited and BiDi characters in the order defined by RFC 3454
     * depending on the options specified in the profile.
     *
     * @param src A string
